...
class ImageBrowser:
    def __init__(self, model, fmc, u1, u2, peco_interval, sta_lta1, sta_lta2, methods_to_show, t=np.array([]), leg_y=1.1, figsize=(16, 10), vnet_thr_inverso=False,
                 gamma=10, gt=True, ci=0, resize=False, titl=''):
        ...
        q = fu.return_example(self.model, self.current_index, self.fmc, self.u1, self.u2, self.peco_interval,
                               self.sta_lta1, self.sta_lta2, vnet_thr_inverso=self.vnet_thr_inverso, resize=self.resize)

        self.m1, self.m2, self.m3, self.m4, self.m5 = self.methos_to_show

        if self.resize:
            # self.t = fu.resize_tof_gt(self.t, 11)
            zoom_factors = (1, 11/16, 11/8) # Compute zoom factors for each dimension
            self.t = zoom(self.t, zoom_factors, order=1) # order = 1 for bilinear interpolation

        # If resize = True it means it will use the original model 11x11, so self.sz = 121
        if self.resize:
            self.sz = 121
        else:
            self.sz = fmc.shape[1] * fmc.shape[2]
        n = np.arange(self.sz)
        ...
        if self.gt:
            if self.t.size != 0:
                ...
                # self.l5 = self.ax[0].plot(self.t[self.current_index, 0,...].flatten(), label='Ground Truth', color='g')
...
class ImageBrowser_modelRegression:
    def __init__(self, fmc, y, t,  ci=0, figsize=(16, 10), titl='', display_activationMaps=False, vmax_in=10):
        # if display_activationMaps=True then y is not the predictions tof indexes predictions, but the activation map of the corresponding layer
        self.current_index = ci
        # self.model = model
        self.fmc = fmc
        self.t = t
        ...
        self.num_images = fmc.shape[0]
        self.fmc = fmc
        self.display_activationMaps = display_activationMaps
        self.fig, self.ax = plt.subplots(1, 2, figsize=figsize)
        self.vmax_in = vmax_in

        self.x = self.fmc[self.current_index, :, :, :, 0]
        self.x = self.x.numpy()
        n_samples = self.x.shape[-1]
        self.x = self.x.reshape((121, n_samples))

        vmax, vmin = self.x.max(), self.x.min()
        gamma = 10

        #-------------------------------------------- Plotting -------------------------------------------------

        self.im1 = self.ax[0].imshow(self.x.T, cmap='seismic')
        self.im1.set_norm(SigmoidNorm(gamma, vmin, vmax))
        self.ax[0].set_aspect(0.1)

        self.ax[0].set_title('Índice actual: {}'.format(str(self.current_index)), fontsize=16)

        self.l1 = self.ax[0].plot(self.t[self.current_index, ...].flatten(), label='Ground Truth', color='g')

        #--------------------------------------------------------------------------------------------------------------#

        if self.display_activationMaps:
            # Plotting the activation map of the corresponding layer

            self.layer_output = self.y[self.current_index, :, :, :, 0]
            # self.y = self.y.numpy()
            n_samples = self.layer_output.shape[-1]
            self.layer_output = self.layer_output.reshape((121, n_samples))

            vmax1, vmin1 = self.layer_output.max(), self.layer_output.min()
            gamma = 10

            self.im2 = self.ax[1].imshow(self.layer_output.T, aspect='auto', cmap='seismic', vmax=self.vmax_in)
            # self.im2.set_norm(SigmoidNorm(gamma, vmin1, vmax1))
            self.ax[1].set_aspect(0.1)
        else:
            # plotting the image with the predicted TOF indexes
            self.im2 = self.ax[1].imshow(self.x.T, cmap='seismic')
            self.im2.set_norm(SigmoidNorm(gamma, vmin, vmax))
            self.ax[1].set_aspect(0.1)
            self.ax[1].set_title(self.titl)

            self.l2 = self.ax[1].plot(self.y[self.current_index], label='Predicted Ground Truth', color='r')
        # --------------------------------------------------------------------------------------------------------------#
    ...
    def update_image(self):
        ...
        # n = np.arange(self.sz)
        self.im1.set_array(self.x.T)
        self.l1[0].set_ydata(self.t[self.current_index, ...].flatten())
        ...
        if self.display_activationMaps:
            ...
            n_samples = self.layer_output.shape[-1]
            self.layer_output = self.layer_output.reshape((121, n_samples))
            self.im2.set_array(self.layer_output.T)
        else:
            self.im2.set_array(self.x.T)
            self.l2[0].set_ydata(self.y[self.current_index])
        # self.l2 = self.ax[1].plot(self.y[self.current_index], label='Predicted Ground Truth', color='g')
