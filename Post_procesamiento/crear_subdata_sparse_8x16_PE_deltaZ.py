...
input_data = fu.crear_input_data(cfg, codepath, envelope=cfg['envelope'], format='npy', dims=[cfg['nel_y'], cfg['nel_x']],
                                 pulseEcho=True)  # n_adq, 8, 16, n_samples

pose_comb = cfg['pose_combinations']
poses = np.zeros((cfg['n_adq'], 6))

delta_z = 79.63  # Utilizado para crear pickle en plano
...
match cfg['forma']:
    case 'plane':
        distfun = ifaz_3d.return_pulse_echo_plane_fun(array_coords_c)
    case 'cylinder':
        distfun = ifaz_3d.return_pulse_echo_cylinder_fun(array_coords_c, cfg['curv_sign'])

# # ------------------------------------------------------------------------------------
poses_c = poses.copy()
# n_disp = len(cfg['tx_list'])  # En caso de FMC parcial con 9 disparos # OJO: si es FMC completa esto tiene que ser 121 (11x11) o 128 (8x16)
n_disp = 1  # Pulse Echo: n_disp = 1
dist = []
nan_idx = []
