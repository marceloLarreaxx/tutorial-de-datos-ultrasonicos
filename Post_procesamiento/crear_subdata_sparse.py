...
input_data = fu.crear_input_data(cfg, codepath, envelope=cfg['envelope'], format='npy', dims=[11,11])  # n_adq, 11, 11, n_samples
...
ascan_elements = list(np.linspace(0, 120, 121).astype(int))  #  ELEMENTOS ACTIVOS DE DISPARO
...
n_disp = 121 # En caso de FMC total
dist = []
nan_idx = []
...
for i in range(cfg['n_adq']):
    if cfg['forma'] == 'cylinder':
        ...
        dist.append(distfun(poses_c[i, :3] + np.array([0, 0, cfg['z_offset']]),
                    ['xyz', poses_c[i, 3:]]).reshape((n_disp, 11, 11)))
    elif cfg['forma'] == 'sphere':
