...
input_data  = fu.crear_input_data(cfg, codepath, envelope=cfg['envelope'], format='npy', dims=[cfg['nel_y'], cfg['nel_x']])  # n_adq, 8, 16, n_samples
...
# delta_z = np.load(cfg['delta_z_corregido_path'])  # PARA LAS TOMAS CON EL DELTA_Z CORREGIDO
# delta_z = 79.21  # Utilizado para crar pickle en cilindro 40 mm u
# delta_z = 80.03  # Utilizado para crear pickle en cilindro 35 mm
# delta_z = 80.03  # Utilizado para crear pickle en plano
# delta_z =  75.08 # Utilizado para crear pickle en plano y cilindro 35 en fmc total
# delta_z =  74.08 # Utilizado para crear pickle en cilindro 12 mm, 40U mm y 25mmU en fmc total
delta_z = 74.58
...
if cfg['forma'] == 'plane':
    ...
    # poses[:, 3:5] = pose_comb[0, 1:3]  # linea solamente para asos que pruebo una sola adquisiciòn PLO
...
# --------------------------ESTE BLOQUE SE LO ACTIVA PARA ADQUISICIONES CON 9 ELEMENTOS ACTIVOS (LOS CASOS COMUNES) --------
# ascan_elements = []
# for tx in cfg['tx_list']:
#     linear_index, _ = u3d.array_ij2element(tx, cfg['nel_x'], cfg['nel_y'], cfg['pitch'], cfg['pitch'])
#     ascan_elements.append(linear_index)

# --------------------------ESTE BLOQUE SE LO ACTIVA PARA ADQUISICIONES TOTALES DE FMC --------
ascan_elements = list(np.linspace(0, 127, 128).astype(int))  #  ELEMENTOS ACTIVOS DE DISPARO
...
# # ------------------------------------------------------------------------------------
poses_c = poses.copy()
# n_disp = len(cfg['tx_list'])  # En caso de FMC parcial con 9 disparos
# n_disp = 1 # para pulse echo
n_disp = 128 # En caso de FMC total
dist = []
nan_idx = []
    ...
    if cfg['forma'] == 'cylinder':
        dist.append(
            distfun(cfg['radio_nominal'], poses_c[i, :3] + np.array([0, 0, cfg['z_offset']]),
                    ['xyz', poses_c[i, 3:]]).reshape((n_disp, cfg['nel_x'], cfg['nel_y'])))
...
# print('1) t_idx: shape = {}'.format(t_idx.shape))
...
# print('2) t_idx: shape = {}'.format(t_idx.shape))
#----------- eliminando índices mayores a 2000 --------
big_idx = []
print('t_idx.shape: {}'.format(t_idx.shape))
...
# print('3) t_idx: shape = {}'.format(t_idx.shape))
...
def plot_algo_2(i, vmax=1000):
    fig, ax = plt.subplots()
    ax.imshow(np.abs(input_data[i, :, :, :]).reshape((128, -1)).T, cmap=arcoiris_cmap, vmax=vmax)
    ax.set_aspect(0.02)
...
def plot_random():
    fig, ax = plt.subplots()
    i, j = np.random.randint(input_data.shape[0]), np.random.randint(8)
...
def plot2(i, j):
    fig, ax = plt.subplots()
    i, j = np.random.randint(subdata['train_fmc'].shape[0]), np.random.randint(8)
