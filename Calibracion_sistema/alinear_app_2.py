...
# ADCAN ELEMENTS FOR ARRAY 11x11
ASCAN_ELEMENTS = [0, 10, 60, 110, 120]
# ADCAN ELEMENTS FOR ARRAY 8X16
# ASCAN_ELEMENTS = [0, 15, 55, 112, 127]
...
## ------------------ ADQUISICIONES FMC TOTALES -------------------------##
# TCP_OFFSET_0 = [30, 0, 262, -180, 0, 0]  # Estimación inicial
# TCP_OFFSET_0 = [28.6, -0.82, 259.85, -180, 0, 0]  # Estimación posterior al ajuste del centro
# TCP_OFFSET_0 = [28.6, -0.82, 259.85, -177.85, 2.95, 0]  # Estimación posterior al primer ajuste de ángulos (xy)
TCP_OFFSET_0 = [28.6, 0.82, 259.85, -177.85, 2.95, 7.24]  # Estimación posterior al segundo ajuste de ángulos (xyz)

#----------------------------------------ARRAY 8X16------------------------------

# TCP inicial para ARRAY 8x16 !!!!
# TCP_OFFSET_0 = [0, 0, 310, -180, 0, 0]  # Primera Aproximación
# TCP_OFFSET_0 = [-5, 0.47, 328.1, -180, 0, 0]  # Primera corrección por mínimos cuadrados (valores de traslación)
# TCP_OFFSET_0 = [-5, 0.47, 328.1, -179.2, 2.6, 0.04]

## Segundo Intento
## ------------------ CON SOPORTE ANTERIOR -------------------------##
# TCP_OFFSET_0 = [-5, 0.17, 320, -180, 0, 0]  # Primera aproximaciòn
# TCP_OFFSET_0 = [5, -0.17, 320, -179.4, 2.8, 7.5]  # Primer Ajuste

## ------------------ CON SOPORTE NUEVO -------------------------##
# TCP_OFFSET_0 = [0, 0, 310, -180, 0, 0] # Primera Aproximación
# TCP_OFFSET_0 = [-5, -1, 326,  -180, 0, 0]  # Primera corrección por mínimos cuadrados (valores de traslación)
# TCP_OFFSET_0 = [5, -1, 326, -179.6, 2.7, 5.0]  # Primer Ajuste
...
PROBE_GAP = 0.75  # (mm)
# PROBE_GAP = 1.85  # para array 8x16 (mm)
...
class LoopWorker(QThread):
    ...
    def run(self):
        ...
        if 0.0 not in theta_y:
            if np.any(theta_y < 0):
                pos_to_neg = np.where(np.diff(np.sign(theta_y)))[0]
        ...
        if self.robotWindowGui.measure_check.isChecked():
            if self.robotWindowGui.sweep_xz_check.isChecked():
                ...
                print("ascan_elements: {}".format(self.ascan_elements))
                ...
                print("ascan_elements: {}".format(ASCAN_ELEMENTS))
        ...
        # REDEFINIMO HOME (POSICIONES PARA REMOVER BURBUJAS)
        z = 57     # Para array 11x11
        # z = 30   # Para array 8x16
        self.robotWindowGui.set_home([self.home[0], self.home[1], z, self.home[3], self.home[4], self.home[5]])
        self.robotWindowGui.robot_interpreter.set_home([self.home[0], self.home[1], z, self.home[3], self.home[4], self.home[5]])
        self.robotWindowGui.go_home()

        time.sleep(10)

        # REDEFINIMO HOME A POSICIÓN DE ADQUISICIÓN
        z = 38  # Para array 11x11
        # z = 30   # Para array 8x16
        self.robotWindowGui.set_home([self.home[0], self.home[1], z, self.home[3], self.home[4], self.home[5]])
...
class FmcView(QMainWindow):
    def __init__(self, alinearGui, robotWindowGui):
        ...
        if self.robotWindowGui.new_home_cylinder_check.isChecked():
            ...
            # PARA EL CASO DE ARRAY 11X11
            home_init = np.load(r'C:\MarceloLarrea\utimag_Marcelo\SITAU_GUIs\Alinear_ST1_IV\Datos Alineamiento Array 11x11\home_delta_z.npy')
            delta_z = np.load(r'C:\MarceloLarrea\utimag_Marcelo\SITAU_GUIs\Alinear_ST1_IV\Datos Alineamiento Array 11x11\delta_z.npy')
            # PARA EL CASO DE ARRAY 8X16
            # home_init = np.load( r'C:\MarceloLarrea\utimag_Marcelo\SITAU_GUIs\Alinear_ST1_IV\Datos Alineamiento Array 8x16\home_delta_z.npy')
            # delta_z = np.load(r'C:\MarceloLarrea\utimag_Marcelo\SITAU_GUIs\Alinear_ST1_IV\Datos Alineamiento Array 8x16\delta_z.npy')
        # delta_z = delta_z.item()
        z = home_init[2]
    ...
    def acquire(self):
        print("DENTRO DE FUNCIÓN acquire() en FmcView")
        ...
        # -------------------------------FOR TOTAL FMC -----------------------------
        self.config_focal_law_fmc()  # FOR TOTAL FMC
        acq_time = self.alinearGui.adquisition_time_spinbox.value()
        gain = self.alinearGui.gain_spinbox.value()
        self.ascan = self.alinearGui.sitau.do_fmc(acq_time, gain)
        print("self.ascan.shape: {}".format(self.ascan.shape))
        i = self.tx_index_spinb.value()
        self.ascan = self.ascan[i, :, :]
        #----------------------------------------------------------------------------
        print("self.ascan.shape: {}".format(self.ascan.shape))
        print(self.ascan[0, :])
        ###################################################################
    ...
    def config_focal_law_fmc(self):
        self.alinearGui.sitau.ST_DeleteFocalLaws()
        self.alinearGui.sitau.ST_SetFullMatrixCapture()
    ...
    def get_tof_2(self, fs=40, c1=1.48):
        ...
        if self.cylinder_check.isChecked():
            ...
            print('z1: {}'.format(z1))
            # --------------------------------------------------
            # z1 = z1 - CYLINDER_RAD  # PARA CILIDRO 35 mm
            # print('CYLINDER_RAD: {}'.format(CYLINDER_RAD))
            #--------------------------------------------------
            z1 = z1 - CYLINDER_RAD_2  # PARA CILINDRO 12 mm
            print('CYLINDER_RAD_2: {}'.format(CYLINDER_RAD_2))
            # --------------------------------------------------
        elif self.cylinder_U_check.isChecked():
            print('delta_z: {}'.format(delta_z))
            z1 = z + delta_z
        ...
        ##### CÁLCULO DE TOF TEÓRICOS #####
        array_coords = np.array(u3d.array_coordinates_list(11, 11, 1, 1))
        # array_coords = np.array(u3d.array_coordinates_list(16, 8, 0.9, 0.9))
        i = self.tx_index_spinb.value()
        array_coords[:, 0] = array_coords[:, 0] * -1  # todo MODIFICAR EN BASE AL TCP OFFSET!!!!!!! rotacion 180 alrededir del eje y
        tx_coords = array_coords[i, :].reshape((1, 3))
        rx_coords = array_coords
        if self.cylinder_check.isChecked():
            if self.interporayos_check.isChecked():
                ...
                fun = ifaz3d.return_pitch_catch_cylfun_circmirror(tx_coords, rx_coords, 1)
            # d = fun(CYLINDER_RAD, np.array([0, xyz_rel[1], z1]), ['xyz', rot_rel])  # PARA CILIDRO 35 mm
            d = fun(CYLINDER_RAD_2, np.array([0, xyz_rel[1], z1]), ['xyz', rot_rel])  # PARA CILIDRO 12 mm
